...
export const Knob = styled.span<KnobProps>((props) => {
	const { isChecked, isDisabled, label, theme } = props;
...
export const Text = styled.span<TextProps>((props) => {
	...
	const { checked, disabled } = (theme as UiKitTheme).switcher.label;
	let color = isDisabled ? disabled.checked.bg : checked.bg;
